    <script>
        ...
        function fillBatteries() {
            ...
            if (isQuantityProblem) {
                ...
                if (!hasDm && (qStr.includes('+') || qStr.includes('-') || (firstNum > 0 && secondNum > 0))) {
                    // Если пример изначально был только в сантиметрах (например, 10 см - 2 см)
                    batteryHintFormula.innerHTML = `
                        <div class="flex flex-col items-center gap-1 font-sans text-center">
                            <div class="text-indigo-300 font-extrabold text-xs sm:text-sm">Считаем числа: ${firstNum} ${op} ${secondNum} = ${ans} см</div>
